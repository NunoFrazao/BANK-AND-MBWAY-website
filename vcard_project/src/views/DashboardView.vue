                ...
                <div class="card">
                    <div class="card-body p-3">
                        <div class="row">
                            <div class="col-8">
                                <div class="numbers">
                                    <p class="text-sm mb-0 text-capitalize font-weight-bold">Vault balance</p>
                                    <h5 class="font-weight-bolder mb-0">
                                        € {{ parseCustomData()?.pig_bank || "0.00" }}
                                    </h5>
                                </div>
                            </div>
                            <div class="col-4 text-end">
                ...
                <div class="card">
                    <div class="card-body p-3">
                        <div class="row">
                            <div class="col-8">
                                <div class="numbers">
                                    <p class="text-sm mb-0 text-capitalize font-weight-bold">Available balance</p>
                                    <h5 class="font-weight-bolder mb-0">
                                        € {{ availableBalance() }}
                                    </h5>
                                </div>
                ...
                <div class="card">
                    <div class="card-body p-3">
                        <div class="row">
                            <div class="col-8">
                                <div class="numbers">
                                    <p class="text-sm mb-0 text-capitalize font-weight-bold">Nº transactions</p>
                                    <h5 class="font-weight-bolder mb-0">
                                        {{ transactionsLength.count }}
                                    </h5>
                                </div>
                            </div>
                            <div class="col-4 text-end">
            ...
            <div class="col-md-4 mb-3">
                <div class="card h-100 mb-4">
                    <div class="card-header pb-0 px-3">
                        <div class="row">
                            <div class="col-md-6">
                                <h6 class="mb-0">Your newest transactions</h6>
                            </div>
                            <div class="col-md-6 d-flex justify-content-end align-items-center">
                                <i class="far fa-calendar-alt me-2"></i>
                                <small>01 - 30 November 2023</small>
                            </div>
                        </div>
                    </div>
                    <div class="card-body pt-4 p-3">
                        <ul class="list-group">
                            <div v-if="transactions.length === 0">
                                <span class="mt-5 small text-secondary">0 resultados encontrados</span>
                            </div>
                            <div v-else>
                                <li v-for="transaction in transactions" :key="transaction.id" :value="transaction.id"
                                    ...
                                    <div class="d-flex align-items-center">
                                        <button
                                            ...
                                            :class="transactionType(transaction) == 'C' ? 'btn-outline-success' : 'btn-outline-danger'"><i
                                                :class="transactionType(transaction) == 'C' ? 'fas fa-arrow-up' : 'fas fa-arrow-down'"></i></button>
                                        <div class="d-flex flex-column">
                                            <h6 class="mb-1 text-dark text-sm">{{ transaction.value }}</h6>
                                            <span class="text-xs">{{ transaction.datetime }}</span>
                                        </div>
                                    </div>
                                    <div class="d-flex align-items-center text-gradient text-sm font-weight-bold"
                                        :class="transactionType(transaction) == 'C' ? 'text-success' : 'text-danger'">
...
const transactionsLength = reactive({ count: 0})
//#endregion
...
const loadTransactions = async () => {
    axios.get(`vcards/${vcardStore.vcard?.phone_number}/transactions`).then((response) => {
        
        // Save in a variable the length of the transactions
        transactionsLength.count = response.data.data.length

        // Save in transactions the first 5 transactions
        transactions.value = response.data.data.slice(0, 5)

        // Accessing custom_options of each transaction in a loop
        transactions.value.forEach((transaction) => {
            // Check if transaction is not null or undefined
            if (transaction && transaction.custom_options) {
...
const parseCustomData = () => {
    const customData = vcardStore.vcard?.custom_data

    try {
        // Check if customData is defined and is a string
        return (typeof customData === 'string')
...
const availableBalance = () => {
    const vcardBalance = vcardStore.vcard?.balance || 0
    const customData = parseCustomData(vcardStore.vcard?.custom_data)
    const pigBank = customData?.pig_bank || 0

    const difference = vcardBalance - pigBank
